...
#
# O ISC DHCP Server dhcpd (uma abreviação de "daemon DHCP") é um programa de servidor DHCP que opera como um daemon 
# em um servidor para fornecer serviço de protocolo de # configuração dinâmica de hosts (DHCP) a uma rede.
# Essa implementação, também conhecida # como ISC DHCP, é uma das primeiras e mais conhecidas, 
# mas agora existem várias outras implementações de software de servidor DHCP disponíveis.
#
