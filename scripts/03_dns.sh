...
#
# O Bind DNS Server BIND (Berkeley Internet Name Domain ou, como chamado previamente, Berkeley Internet Name Daemon) 
# é o servidor para o protocolo DNS mais utilizado na Internet, especialmente em sistemas do tipo Unix, onde ele pode 
# ser considerado um # padrão de facto. Foi criado por quatro estudantes de graduação, membros de um grupo de pesquisas 
# em ciência da computação da Universidade de Berkeley, e foi distribuído pela primeira vez com o sistema 
# operacional 4.3 BSD. Atualmente o BIND é suportado e mantido pelo Internet Systems Consortium.
#
...
#	nslookup .corp (query Internet name servers interactively)
#	dig .corp (DNS lookup utility)
#	host .corp (DNS lookup utility)
#	ping .corp (send ICMP ECHO_REQUEST to network hosts)
#
# Windows Powershell.: 
#	nslookup .corp
#	nslookup 182.13.1.10
#	nslookup srvsp01ws01
#	nslookup srvsp01ws01..corp
#	ipconfig /displaydns
#	ping .corp
#	Resolve-DnsName .corp
#	Test-Connection .corp
#
# Arquivo de configuração dos parâmetros utilizados nesse script
source _parametros.sh
#
...
if [ "$UBUNTU" == "20.04" ]
	then
		...
		echo -e "Distribuição não é >= 20.04.x ($UBUNTU)"
		exit 1
fi
#
########################################################################################################################
...
echo -e "o valor da variável NETPLAN no arquivo de configuração: _parametros.sh"
	# opção do comando read: -s (Do not echo keystrokes)
	read -s
	vim $NETPLAN
echo -e "Arquivo editado com sucesso!!!, continuando com o script...\n"
...
echo -e "Atualizando os arquivos de configuração do Bind DNS Server, aguarde..."
	...
	cp -v conf/dns/{.corp.hosts,182.13.1.rev} /var/lib/bind/ &>> $LOG
	...
	chown -v root:bind /var/lib/bind/{.corp.hosts,182.13.1.rev} &>> $LOG
...
#
########################################################################################################################
echo -e "Editando o arquivo de configuração .corp.hosts, pressione <Enter> para continuar."
	# opção do comando: &>> (redirecionar a saída padrão)
	# opção do comando read: -s (Do not echo keystrokes)
	read -s
	vim /var/lib/bind/.corp.hosts
	named-checkzone $DOMAIN /var/lib/bind/.corp.hosts &>> $LOG
echo -e "Arquivo editado com sucesso!!!, continuando com o script...\n"
sleep 5
#
########################################################################################################################
echo -e "Editando o arquivo de configuração 182.13.1.rev, pressione <Enter> para continuar."
	# opção do comando: &>> (redirecionar a saída padrão)
	# opção do comando read: -s (Do not echo keystrokes)
	read -s
	vim /var/lib/bind/182.13.1.rev
	named-checkzone $DOMAINREV /var/lib/bind/182.13.1.rev &>> $LOG
	named-checkzone $NETWORK /var/lib/bind/182.13.1.rev &>> $LOG
echo -e "Arquivo editado com sucesso!!!, continuando com o script...\n"
sleep 5
#
########################################################################################################################
