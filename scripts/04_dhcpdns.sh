...
#
# O Bind DNS Server BIND (Berkeley Internet Name Domain ou, como chamado previamente, Berkeley Internet Name Daemon) é 
# o servidor para o protocolo DNS mais utilizado na Internet, especialmente em sistemas do tipo Unix, onde ele pode ser 
# considerado um padrão de facto. Foi criado por quatro estudantes de graduação, membros de um grupo de pesquisas em 
# ciência da computação da Universidade de Berkeley, e foi distribuído pela primeira vez com o sistema operacional 
# 4.3 BSD. Atualmente o BIND é suportado e mantido pelo Internet Systems Consortium.
#
# O ISC DHCP Server dhcpd (uma abreviação de "daemon DHCP") é um programa de servidor DHCP que opera como um daemon em 
# um servidor para fornecer serviço de protocolo de configuração dinâmica de hosts (DHCP) a uma rede. Essa implementação, 
# também conhecida como ISC DHCP, é uma das primeiras e mais conhecidas, mas agora existem várias outras implementações 
# de software de servidor DHCP disponíveis.
#
...
echo -n "Verificando as dependências da Integração do DHCP e DNS Server, aguarde... "
	...
	do
		...
		[[ $deps -ne 1 ]] && echo "Dependências.: OK" || { 
			...
			echo -en "Recomendo utilizar o script: 02_dhcp.sh para resolver as dependências"
			echo -en "Recomendo utilizar o script: 03_dns.sh para resolver as dependências."
            exit 1; 
            }
		sleep 5
#
